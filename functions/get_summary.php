<?php
	...
	function get_summary($table)
	{
		...
		foreach($rows as $financial_transaction)
		{
			...
			echo '<tr><td contenteditable onBlur="saveToDb(this,\'day_of_month\')">'.$financial_transaction['day_of_month'].'</td>';
			echo '<td contenteditable onBlur="saveToDb(this,\'description\')">'.$financial_transaction['description'].'</td>';
			echo '<td contenteditable onBlur="saveToDb(this,\'amount\')">'.$financial_transaction['amount'].'</td></tr>';
		}
		echo "</table>";
		echo "</td></tr><tr><td align='center'><a href='../index.php'>Home</a></td></tr></table>";
	}
	include '../ajax/db_functions.js';
?>
